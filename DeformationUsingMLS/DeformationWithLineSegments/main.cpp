...
int row,column;
int numOfControlLine;
vector<Segment> myOldControlLine,myNewControlLine;
vector<Point> mycommonPoint;
...
void DrawMesh()//画网格
{
	...
	//画出控制线段形变前后的位置,对算法并无影响,可以删去
	for(i=0;i<myOldControlLine.size();i++)
	{
		glBegin(GL_LINE_LOOP);
		glVertex3f(myOldControlLine[i].pa.xx*0.5-25,myOldControlLine[i].pa.yy*0.5-25,0);
		glVertex3f(myOldControlLine[i].pb.xx*0.5-25,myOldControlLine[i].pb.yy*0.5-25,0);
		glEnd();
	}
	for(i=0;i<myOldControlLine.size();i++)
	{
		glBegin(GL_LINE_LOOP);
		glVertex3f(myNewControlLine[i].pa.xx*0.5-25,myNewControlLine[i].pa.yy*0.5-25,0);
		glVertex3f(myNewControlLine[i].pb.xx*0.5-25,myNewControlLine[i].pb.yy*0.5-25,0);
		glEnd();
	}
}
...
void printInputInfo()
{
	...
	cout<<"第一行2个int型数据,表示网格的高和宽"<<endl
		<<"第二行1个int型数据n,表示控制线段的个数"<<endl
		<<"接下来n行每行8个int型数据:x0,y0,x1,y1,x2,y2,x3,y3,"<<endl
		<<"表示控制线段{(x0,y0),(x1,y1)}形变后所处位置为{(x2,y2),(x3,y3)}"<<endl;
}
...
void main(int argc, char** argv)
{
	...
	scanf("%d",&numOfControlLine);
	for(i=0;i<numOfControlLine;i++)
	{
		scanf("%lf%lf%lf%lf%lf%lf%lf%lf",&ts1.pa.xx,&ts1.pa.yy,&ts1.pb.xx,&ts1.pb.yy,&ts2.pa.xx,&ts2.pa.yy,&ts2.pb.xx,&ts2.pb.yy);
		/*
		ts1.pa.yy=99-ts1.pa.yy;
		ts1.pb.yy=99-ts1.pb.yy;
		ts2.pa.yy=99-ts2.pa.yy;
		ts2.pb.yy=99-ts2.pb.yy;
		*/
		myOldControlLine.push_back(ts1);
		myNewControlLine.push_back(ts2);
	}
	mycommonPoint.clear();
	for(i=0;i<row;i++)
	{
	...
	AffineLinesFast myAffineDeformation(myOldControlLine);
	myAffineDeformation.UpdateControlLine(myNewControlLine);
	myAffineDeformation.UpdateCommonPoint(mycommonPoint);
	myAffineDeformation.getResultofCommonPoint(printAffinePoint);
	SimilarityLinesFast mySimilarityDeformation(myOldControlLine);
	mySimilarityDeformation.UpdateControlLine(myNewControlLine);
	...
	RigidLinesFast myRigidDeformation(myOldControlLine);
	myRigidDeformation.UpdateControlLine(myNewControlLine);
	...
	glutInitWindowPosition(200, 250);  // Set window position on screen
