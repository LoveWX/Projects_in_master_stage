...
SimilarityLinesFast::SimilarityLinesFast()
{
...
SimilarityLinesFast::SimilarityLinesFast(vector<Segment> &oldCS)
{
...
void SimilarityLinesFast::UpdateControlLine(vector<Segment> &newCS)
{
	newControlLine=newCS;
	isComputed=false;
}

void SimilarityLinesFast::UpdateCommonPoint(vector<Point> &newCommonPoint)
{
...
void SimilarityLinesFast::calcA()
{
	int i,j;
	double td1,td2;
	Point tp1,tp2,tp3;
	...
	us=new double[commonPoint.size()];
	...
	for(i=0;i<commonPoint.size();i++)
	{
		...
		for(j=0;j<oldControlLine.size();j++)
		{
			...
			us[i]+=DotProduct(tp1,tp1)*tempVector00[j];
			us[i]+=DotProduct(tp1,tp2)*tempVector01[j]*2.0;
			us[i]+=DotProduct(tp2,tp2)*tempVector11[j];
		}
...
int SimilarityLinesFast::getNumOfControlLine()const
{
	return oldControlLine.size();
}

void SimilarityLinesFast::getResultofCommonPoint(vector<Point> &resultPoint)
{
	if(!isComputed)
	{
		doSimilarityLinesFast();
	}
	resultPoint=ansPoint;
}

void SimilarityLinesFast::productofMatrix4442()
{
	int i,j;
	double temp[4][2];
	for(i=0;i<4;i++)
...
void SimilarityLinesFast::productofMatrix4222()
{
	int i;
	double temp[4][2];
	for(i=0;i<4;i++)
...
void SimilarityLinesFast::cleartempMatrix()
{
	int i,j;
	for(i=0;i<4;i++)
	{
...
void SimilarityLinesFast::doubleswap(double &a,double &b)
{
	double temp=a;
	a=b;
	b=temp;
...
double SimilarityLinesFast::doubleabs(double a)const
{
	if(a<0)
	{
		return -a;
...
int SimilarityLinesFast::isOnControlLine(const Point &thepoint)
{
	int i;
	Point tva,tvb;
	double dta,dtb;
...
void SimilarityLinesFast::doSimilarityLinesFast()
{
	...
	for(i=0;i<commonPoint.size();i++)
	{
		...
		for(j=0;j<oldControlLine.size();j++)
		{
			...
			thePoint.xx+=(tp1.xx*A[i][j].Aj[0][0]+tp1.yy*A[i][j].Aj[1][0]+tp2.xx*A[i][j].Aj[2][0]+tp2.yy*A[i][j].Aj[3][0])/us[i];
			thePoint.yy+=(tp1.xx*A[i][j].Aj[0][1]+tp1.yy*A[i][j].Aj[1][1]+tp2.xx*A[i][j].Aj[2][1]+tp2.yy*A[i][j].Aj[3][1])/us[i];
		}
