...
{
public:
	SimilarityDeformationFast();									//默认构造函数，需指定newControlPoint和commonPoint之后才能进行形变
	SimilarityDeformationFast(vector<Point> &oldCP);				//含参构造函数，需指定newControlPoint和commonPoint之后才能进行形变
	~SimilarityDeformationFast();									//析构函数
	...
	void getResultofCommonPoint(vector<Point> &resultPoint);		//调用doSimilarityDeformation()进行形变并将结果拷贝到resultPoint中
protected:
	...
	void doSimilarityDeformation();									//调用预计算结果对所有非控制点进行形变，用形变结果更新commonPoint向量
