...
{
	vector<string> names; 
	string inDir;//需要处理的图片所在的文件夹
	int imgNum = CmFile::GetNamesNE(imgNameW, names, inDir);//判断文件夹inDir里有几幅图片要处理
	CmFile::MkDir(salFileDir);//创建文件夹salFileDir
	...
	for (int i = 0; i < imgNum; i++)//每一副图片都要做
	{
		CmLog::LogProgress("Processing %d/%dth image: %-40s\r", i, imgNum, names[i].c_str());

		Mat sal, img3f = imread(inDir + names[i] + ".jpg");//img3f读入要处理的那副图片 sal备用，作为每一次的输出图片
