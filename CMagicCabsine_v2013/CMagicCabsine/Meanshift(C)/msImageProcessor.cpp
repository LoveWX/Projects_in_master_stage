...
void msImageProcessor::DefineImage(_byte *data_, imageType type, int height_, int width_)
{
...
void msImageProcessor::DefineBgImage(_byte* data_, imageType type, int height_, int width_)
{
...
void msImageProcessor::RGBtoLUV(_byte *rgbVal, float *luvVal)
{
...
void msImageProcessor::LUVtoRGB(float *luvVal, _byte *rgbVal)
{
	//declare variables...
	int		r, g, b;
...
/*        outputImage.                                 */
/*******************************************************/
void msImageProcessor::GetResults(_byte *outputImage)
{
	...
	//if the image type is GREYSCALE simply copy it over to the segmentedImage
	if(N == 1)
	{
		//copy over msRawData to segmentedImage checking bounds
		int i, pxValue;
		for(i = 0; i < L; i++)
		{
			//get value
			...
			if(pxValue < 0)
				outputImage[i] = (_byte)(0);
			else if(pxValue > 255)
				outputImage[i] = (_byte)(255);
			else
				outputImage[i] = (_byte)(pxValue);
		}
	}
	else if (N == 3)
	{		
...
void msImageProcessor::DefineBoundaries( void )
{
	...
	//define boundaries for all rows except for the first and last one...
...
void msImageProcessor::InitializeOutput( void )
{
	...
	if((!(modes = new float [L*(N+2)]))||(!(labels = new int [L]))||(!(colLabels = new unsigned char [L*3]))||(!(modePointCounts = new int [L]))||(!(indexTable	= new int [L])))
	{
		ErrorHandler("msImageProcessor", "Allocate", "Not enough memory");
		return;
	}
...
void msImageProcessor::SetColorLabels(void)
{
...
void msImageProcessor::GetLabels(int* mylabels)
{
	memcpy(mylabels,labels,width*height*sizeof(int));
}
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
