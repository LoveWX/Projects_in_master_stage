...
public:
	...
	void SetAreaMatrix(const vector< vector<int> > &theMatrix);//设置当前区域矩阵
	...
	void getAreaMatrix(vector< vector<int> > &areaMat);//获取当前区域矩阵
	void setAreaMatrix(const vector< vector<int> > &areaMat);//设置当前区域矩阵
...
ImageRegionOperator::ImageRegionOperator(IplImage *theImage,const vector< vector<int> > &theMatrix)
{
...
void ImageRegionOperator::SetImageAndMatrix(IplImage *theImage,const vector< vector<int> > &theMatrix)
{
	if(srcImage!=NULL)
	{
		cvReleaseImage(&srcImage);
...
void ImageRegionOperator::SetAreaMatrix(const vector< vector<int> > &theMatrix)
{
	areaMatrix=theMatrix;
}
...
void ImageRegionOperator::printImage()
{
	CalcAverageColor();
	int ii,jj;
	for(ii=0;ii<height;ii++)
	{
		...
		{
			if(areaMatrix[ii][jj]==maxRegionNum)
			{
				cvSet2D(displayImage,ii,jj,cvScalar(0,0,0));
			}
		}
	}
	cvSaveImage("aa.bmp",displayImage);
}

void ImageRegionOperator::testMasks(vector< vector<int> > &masks)
{
	int i,j;
	int row=masks.size();
	int col=masks[0].size();
	IplImage *img=cvCreateImage(cvSize(col,row),displayImage->depth,displayImage->nChannels);
	...
	for(i=0;i<row;i++)
	{
		for(j=0;j<col;j++)
		{
			...
			{
				cvSet2D(img,i,j,cvScalar(0,0,255));
			}
			else if(masks[i][j]==3)
			{
			...
			{
				cvSet2D(img,i,j,cvScalar(255,0,0));
			}
			else
			{
				cvSet2D(img,i,j,cvScalar(255,138,128));
			}
		}
	}
	cvSaveImage("mask.bmp",img);
}

void ImageRegionOperator::testRegion(vector< vector<int> > &theMatrix)
{
	IplImage *img=cvCloneImage(displayImage);
	int i,j;
	for(i=0;i<height;i++)
	{
		for(j=0;j<width;j++)
		{
			...
			else if(theMatrix[i][j]==maxRegionNum)
			{
				cvSet2D(img,i,j,cvScalar(0,0,0));
			}
			...
			else if(theMatrix[i][j]==maxRegionNum-1)
			{
				cvSet2D(img,i,j,cvScalar(255,0,0));
			}
			else
	...
	cvSaveImage("matrix.bmp",img);
}

void ImageRegionOperator::printMatrix(vector< vector<int> > &theMatrix)
{
	int i,j;
	printf("thematrix is:\n");
	for(i=0;i<height;i++)
